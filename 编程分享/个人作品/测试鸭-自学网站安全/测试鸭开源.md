...
这个网站可不是传统的教程网站，挂个课、挂个电子书就结束了，而是一个 **交互式** 自学网站。WBGaps结合自己遭受网站攻击的经历和教训，给网站设置了 30 多个漏洞。
...
每当你找到一个 Bug，站长WBGaps的血鸭值都会极速上升，并且你还可以看到WBGaps给出的小知识点，以及一张WBGaps的高清无码发飙图，据说总共有 5 张，集齐之后也许不能召唤神龙，但WBGaps会请你喝茶。
...
const doGameUnitSucceed = (key: string) => {
  ...
  setTimeout(() => {
    Modal.success({
      title: `太棒了，WBGaps的血鸭又高了！😡 ${gameState.score - unit.score} +${unit.score}`,
      content: ...,
      okText: '继续加油！',
    });
  }, 1000);
